...
class SaeTrainer:
    def __init__(
        ...
        for hook in self.local_hookpoints():
            for seed in cfg.init_seeds:
                ...
                # print("Sae encoder shape:", self.saes[name].encoder.weight.shape)
                # print("Top-k:", self.saes[name].cfg.k)
                # print("Num latents:", self.saes[name].num_latents)
    ...
    def fit(self):
        ...
        def hook(module: nn.Module, inputs, outputs):
            # print("Hook triggered for", module)
        ...
        for i in range(5):
            for batch in dl:
                ...
                pixel_values = batch["input_ids"].to(device)
                N = pixel_values.shape[0] * pixel_values.shape[1] * pixel_values.shape[2] * pixel_values.shape[3]
                num_tokens_in_step += N
                # print("num_tokens_in_step =", num_tokens_in_step)
                ...
                try:
                    with torch.no_grad():
                        ...
                        _ = self.model.vision_model(pixel_values)
                finally:
                ...
                for name, raw in self.saes.items():
                    ...
                    for in_chunk, out_chunk in zip(in_chunks, out_chunks):
                        # print("NUM: ", max(self.num_tokens_since_fired[name]))
                        ...
                        ##########################################################################################
                        used_latents = out.latent_indices.flatten().tolist()

                        for idx in used_latents:
                            usage_counter[idx] += 1
                        ...
                        ##########################################################################################
                        clear_output()
                        print(float(out.fvu), float(self.cfg.auxk_alpha * out.auxk_loss))
                        used_latents = out.latent_indices.flatten().tolist()

                        for idx in used_latents:
                            usage_counter[idx] += 1
            
                        # Continue your normal training steps, etc.
                
                        # After the loop (or periodically), see how many latents got used
                        total_used = len(usage_counter)
                        print(f"{total_used} latents out of {raw.num_latents} were used at least once.")
                    
                        # Optionally, see a histogram or distribution:
                        sorted_usage = sorted(usage_counter.items(), key=lambda x: x[1], reverse=True)
                        print("Top 10 most-used latents:", sorted_usage[:10])
                        print("Bottom 10 least-used latents:", sorted_usage[-10:])
                        
                        track_loss.append(float(loss))
                        plt.plot(range(len(track_loss)), track_loss)
                        plt.title(f"Loss Tracking - Epoch {i + 1}")
                        plt.xlabel("Batch")
                        plt.ylabel("Loss")
